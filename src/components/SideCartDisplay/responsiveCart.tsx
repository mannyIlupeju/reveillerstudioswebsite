...
export default function SideCart() {
  ...
  const {setIsCartOpen, isCartOpen, openCart} = useGlobalContext();
  ...
  return (
    ...
    className={`
      fixed z-20 shadow-lg flex flex-col glassBox transition
      ${isMobile ? 'bottom-0 left-0 w-full max-h-[90vh]' : 'top-0 right-0 xl:w-[30vw] lg:w-[80vw] h-screen'}
      bg-gray-200
    `}
    >
        ...
        <div className="flex-1 overflow-y-auto p-6 cursor-pointer justify-center">
          {cartItems.length !== 0 ? cartItems.map((item) => (
              ...
              >
                 <div className="xl:w-full w-48 relative ">
                  <Image
                      src={item.image}
                      fill
                      priority
                      alt={item.title}
                      className="xl:object-contain xl:aspect-ratio[4/5] aspect-ratio[2/3] object-contain w-fit "
                  />
                  </div>
